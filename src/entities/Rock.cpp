...
void Rock::draw() {
    ...
    if (m_Active && !m_Distroyed) {
        ...
        for (int point = 0; point < 11; point++) {
            rockLine.start =
                Vector2i((int)(position.x + m_RockPoints[point].x), (int)(position.y + m_RockPoints[point].y));
            rockLine.end =
                Vector2i((int)(position.x + m_RockPoints[point + 1].x), (int)(position.y + m_RockPoints[point + 1].y));
            Window::DrawVolumetricLine(&rockLine, m_Color);
        }

        rockLine.start = Vector2i((int)(position.x + m_RockPoints[11].x), (int)(position.y + m_RockPoints[11].y));
        rockLine.end = Vector2i((int)(position.x + m_RockPoints[0].x), (int)(position.y + m_RockPoints[0].y));
        Window::DrawVolumetricLine(&rockLine, m_Color);
    }
...
void Rock::drawDustExplosion() {
    ...
    for (int i = 0; i < 12; i++) {
        ...
        Window::DrawVolumetricLine(&dustLine, particle.color);
        ...
        if (particle.color.alpha > 50) {
            ...
            Window::DrawVolumetricLine(&dustLine, fadedColor);
...
void Rock::checkForEdge() {
    ...
    if (m_Location.x - rockRadius < borderOffset ||
        m_Location.x + rockRadius > Window::GetWindowSize().x - borderOffset ||
        ...
        bool hitOutsideBorder = (m_Location.x - rockRadius < borderOffset ||
                                 m_Location.x + rockRadius > Window::GetWindowSize().x - borderOffset);
    ...
    if (m_Location.y - rockRadius < borderOffset ||
        m_Location.y + rockRadius > Window::GetWindowSize().y - borderOffset ||
        (m_Location.x + rockRadius > InsideLeft && m_Location.x - rockRadius < InsideRight &&
         m_Location.y + rockRadius > InsideTop && m_Location.y - rockRadius < InsideBottom)) {

        // Only bounce if hitting outside borders or StatusDisplay border
        bool hitOutsideBorder = (m_Location.y - rockRadius < borderOffset ||
                                 m_Location.y + rockRadius > Window::GetWindowSize().y - borderOffset);
        bool hitStatusDisplay = (m_Location.x + rockRadius > InsideLeft && m_Location.x - rockRadius < InsideRight &&
                                 m_Location.y + rockRadius > InsideTop && m_Location.y - rockRadius < InsideBottom);
