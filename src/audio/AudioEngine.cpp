...
void AudioEngine::LoadSound(const std::string &sSoundName, bool b3d, bool bLooping, bool bStream)
{
    ...
    std::string path = Window::dataPath() + "/" + sSoundName + ".wav";
    AudioEngine::ErrorCheck(mImplementation->mSystem->createSound(path.c_str(), eMode, nullptr, &sound));
    if (sound)
    {
...
void AudioEngine::LoadBank(const std::string &sBankName, FMOD_STUDIO_LOAD_BANK_FLAGS pflags)
{
    ...
    std::string path = Window::dataPath() + "/" + sBankName;
