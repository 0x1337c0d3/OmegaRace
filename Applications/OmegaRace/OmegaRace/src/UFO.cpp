...
UFO::UFO() : radius(25.0f), destroyed(false), active(false), width(40.0f), 
             directionTimer(0.0f), directionDuration(3.0f), fromLeft(true),
             explosionActive(false), explosionTimer(0.0f), explosionDuration(3.0f) {
    color.red = 0;
    color.green = 255;
    color.blue = 255;
    color.alpha = 255;
...
void UFO::buildUFO() {
    // Build classic UFO shape - 10 lines forming a saucer
    float w = width;
    
    // Top small section
    ufoLines[0][0] = omegarace::Vector2i(-(int)(w / 7), -(int)(w / 5));
    ufoLines[0][1] = omegarace::Vector2i((int)(w / 7), -(int)(w / 5));
    
    // Upper middle section
    ufoLines[1][0] = omegarace::Vector2i(-(int)(w / 4), 0);
    ufoLines[1][1] = omegarace::Vector2i((int)(w / 4), 0);
    
    // Lower middle section (widest)
    ufoLines[2][0] = omegarace::Vector2i(-(int)(w / 2), (int)(w / 6));
    ufoLines[2][1] = omegarace::Vector2i((int)(w / 2), (int)(w / 6));
    
    // Bottom section
    ufoLines[3][0] = omegarace::Vector2i(-(int)(w / 3), (int)(w / 3));
    ufoLines[3][1] = omegarace::Vector2i((int)(w / 3), (int)(w / 3));
    
    // Connecting lines - left side
    ufoLines[4][0] = ufoLines[0][0]; // Top left to upper middle left
    ufoLines[4][1] = ufoLines[1][0];
    
    // Connecting lines - right side
    ufoLines[5][0] = ufoLines[0][1]; // Top right to upper middle right
    ufoLines[5][1] = ufoLines[1][1];
    
    // Left side curves
    ufoLines[6][0] = ufoLines[1][0]; // Upper middle left to lower middle left
    ufoLines[6][1] = ufoLines[2][0];
    
    // Right side curves
    ufoLines[7][0] = ufoLines[1][1]; // Upper middle right to lower middle right
    ufoLines[7][1] = ufoLines[2][1];
    
    // Bottom left curve
    ufoLines[8][0] = ufoLines[2][0]; // Lower middle left to bottom left
    ufoLines[8][1] = ufoLines[3][0];
    
    // Bottom right curve
    ufoLines[9][0] = ufoLines[2][1]; // Lower middle right to bottom right
    ufoLines[9][1] = ufoLines[3][1];
}
...
void UFO::update(double frame) {
    ...
    if ((fromLeft && position.x > omegarace::Window::GetWindowSize().x + 50) ||
        (!fromLeft && position.x < -50)) {
        active = false;
    }
...
void UFO::draw() {
    ...
    if (active && !destroyed) {
        ...
        for (int line = 0; line < 10; line++) {
            ufoLine.start = Vector2i((int)(position.x + ufoLines[line][0].x), 
                                           (int)(position.y + ufoLines[line][0].y));
            ufoLine.end = Vector2i((int)(position.x + ufoLines[line][1].x), 
                                         (int)(position.y + ufoLines[line][1].y));
...
void UFO::triggerExplosion() {
    if (explosionActive) return; // Already exploding
